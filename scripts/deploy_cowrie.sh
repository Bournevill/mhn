...
cd /opt
git clone https://github.com/Bournevill/cowrie.git cowrie
cd cowrie
...
sed -i 's/ssh_version_string = SSH-2.0-OpenSSH_6.0p1 Debian-4+deb7u2/ssh_version_string = SSH-2.0-OpenSSH_6.6p1 Ubuntu-2ubuntu2.6/g' cowrie.cfg
...
sed -i 's/AUTHBIND_ENABLED=no/AUTHBIND_ENABLED=yes/' /opt/cowrie/bin/cowrie
sed -i 's/DAEMONIZE=""/DAEMONIZE="-n"/' /opt/cowrie/bin/cowrie
